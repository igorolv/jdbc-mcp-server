-- Usage catalog schema (SQLite). Stores known SQL queries used by applications/reports
-- against the inspected database, with their business context. Local-only writes; the
-- inspected JDBC database is never touched by this catalog.
...
CREATE TABLE IF NOT EXISTS query (
    ...
    resolved_snapshot_version INTEGER
);
...
CREATE TABLE IF NOT EXISTS query_tag (
    query_uid TEXT NOT NULL REFERENCES query(uid) ON DELETE CASCADE,
    tag       TEXT NOT NULL,
    PRIMARY KEY (query_uid, tag)
);
...
CREATE INDEX IF NOT EXISTS idx_query_param_query ON query_param(query_uid);
...
CREATE INDEX IF NOT EXISTS idx_query_table_query ON query_table(query_uid);

CREATE TABLE IF NOT EXISTS query_column (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    query_uid       TEXT NOT NULL REFERENCES query(uid) ON DELETE CASCADE,
    query_table_id  INTEGER REFERENCES query_table(id) ON DELETE SET NULL,
    schema_resolved TEXT,
    table_resolved  TEXT,
...
CREATE INDEX IF NOT EXISTS idx_query_column_query ON query_column(query_uid);
...
CREATE TABLE IF NOT EXISTS query_output (
    ...
    business_label       TEXT,
    business_description TEXT,
    UNIQUE (query_uid, alias)
);

CREATE TABLE IF NOT EXISTS query_output_column (
    query_output_id INTEGER NOT NULL REFERENCES query_output(id) ON DELETE CASCADE,
    schema_resolved TEXT,
    table_resolved  TEXT,
    column_name     TEXT NOT NULL,
    PRIMARY KEY (query_output_id, schema_resolved, table_resolved, column_name)
);
...
CREATE TABLE IF NOT EXISTS query_field_usage (
    id                         INTEGER PRIMARY KEY AUTOINCREMENT,
    query_uid                  TEXT NOT NULL REFERENCES query(uid) ON DELETE CASCADE,
    query_output_id            INTEGER REFERENCES query_output(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_qfu_query ON query_field_usage(query_uid);
