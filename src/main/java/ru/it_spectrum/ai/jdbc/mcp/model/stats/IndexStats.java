...
public record IndexStats(
    ...
    public record IndexStatsRow(
            ...
            long sizeBytes,
            @Schema(description = "Number of recorded index scans since statistics were last reset, when available.", nullable = true, requiredMode = Schema.RequiredMode.NOT_REQUIRED)
            Long idxScans,
            @Schema(description = "Number of index tuples read according to engine statistics, when available.", nullable = true, requiredMode = Schema.RequiredMode.NOT_REQUIRED)
            Long idxTupRead,
            ...
            Long idxTupFetch,
            @Schema(description = "Oracle distinct key count for the index, when available.", nullable = true, requiredMode = Schema.RequiredMode.NOT_REQUIRED)
            Long distinctKeys,
            @Schema(description = "Oracle clustering factor for the index, when available.", nullable = true, requiredMode = Schema.RequiredMode.NOT_REQUIRED)
            Long clusteringFactor,
            @Schema(description = "Oracle B-tree level for the index, when available.", nullable = true, requiredMode = Schema.RequiredMode.NOT_REQUIRED)
            Integer blevel,
            @Schema(description = "Oracle leaf block count for the index, when available.", nullable = true, requiredMode = Schema.RequiredMode.NOT_REQUIRED)
            Long leafBlocks,
