...
public record QueryLintResult(
        @Schema(description = "Parsed query inspection that underpins lint (opaque; the inspectQuery tool returns the typed form).", nullable = true, requiredMode = Schema.RequiredMode.NOT_REQUIRED)
    ...
    public static QueryLintResult parserError(QueryInspection inspection, QueryWarning warning) {
        return new QueryLintResult(Opaque.of(inspection), false, null, null, List.of(warning));
    }
}
